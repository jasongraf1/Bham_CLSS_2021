...
```
You can find the latitude and longitude of those tweets that have them.
...
# read in a small dataframe of local cities
...
```
It's not the best, but there is a lot more you could do if you're keen on using geolocated data.  


<!-- There's a lot of data here. Even if we just focus on the UK, we can see there's way too much information. -->
...
<!-- So we'll simplify things by finding the maximal bounds for our coordinates -->
...
<!-- We can use these as a guide to set the limits of our map like so. -->
...
<!--   coord_fixed( -->
<!--     ratio = 1.3, # set the aspect ratio -->
<!--     xlim = c(-10,3),  -->
<!--     ylim = c(50.3, 59) -->
<!--     ) + -->
<!--   theme(panel.background = element_rect(fill = "lightblue")) -->
