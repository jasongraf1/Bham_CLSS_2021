...
The first thing we'll do is simply get the titles of every book listed on this first page (we'll ignore the other pages for now). `{rvest}` can do this for us, but first we must know how to identify the correct element on the page. What we need to do is find the correct CSS selector for the book titles. We'll do this using the Selector Gadget by following these steps (or watch my [**short video tutorial**]() on the BCLSS Canvas page):
